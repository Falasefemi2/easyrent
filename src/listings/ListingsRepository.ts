...
const toListingRow = (row: ListingSelectRow, favoriteCount = 0): ListingRow => ({
  id: row.id,
...
const toMediaRow = (row: typeof listingMedia.$inferSelect): ListingMediaRow => ({
  id: row.id,
...
export class ListingRepository extends Context.Service<
  ListingRepository,
  {
    ...
    readonly findByLandlord: (landlordId: string, pagination: PaginationParams) => DbEffect<PaginatedResult<ListingRow>>
      ...
      const findById = Effect.fn("ListRepository.findById")(
        (id: string): DbEffect<Option.Option<ListingRow>> =>
          Effect.gen(function* () {
            ...
            return Option.some(toListingRow(rows[0], Number(countRows[0]?.count ?? 0)))
          }),
      )

      const findByIdWithMedia = Effect.fn("ListingRepository.findByIdWithMedia")(
        (
          id: string,
        ): DbEffect<
          Option.Option<
        ...
        > =>
          Effect.gen(function* () {
            const [rows, media, countRows] = yield* Effect.all(
              [
                withDbSpan(
      ...
      const findAll = Effect.fn("ListingsRepository.findAll")(
        (
          ...
          Effect.gen(function* () {
            ...
            return {
              data: rows.map((r) => toListingRow({ ...r, coverImage: coverMap[r.id] ?? null }, countMap[r.id] ?? 0)),
              total,
              page,
              limit,
      ...
      const update = Effect.fn("ListingsRepository.update")(
        (
          ...
          Effect.gen(function* () {
            ...
            if (params.furnished !== undefined) updateData.furnished = params.furnished
