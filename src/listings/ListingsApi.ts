...
import { ListingForbidden, ListingMediaError, ListingNotFound } from "./ListingsError"
...
export class ListingsApiGroup extends HttpApiGroup.make("listings")
  .add(
    HttpApiEndpoint.get("list", "/listings", {
      query: Schema.Struct({
        ...
        status: Schema.optional(Schema.Literals(["avaiable", "rented", "inative"])),
  ...
  .add(
    HttpApiEndpoint.post("uploadMedia", "/listings/:id/media", {
      ...
      error: [ListingNotFound, ListingForbidden, ListingMediaError, ImageUploadError],
    }).middleware(Authorization),
  )
  .add(
