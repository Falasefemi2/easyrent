...
import { type ImageUploadError, ImageUploadService } from "../services/UploadThingService"
...
export class ListingService extends Context.Service<
  ListingService,
  {
    ...
    readonly uploadMedia: (params: {
      ...
      type: "image" | "video"
      order: number
    }) => Effect.Effect<ListingMediaRow, ListingNotFound | ListingForbidden | ImageUploadError>
    ...
    readonly delete: (id: string, landlordId: string) => Effect.Effect<void, ListingNotFound | ListingForbidden>
    ...
    Effect.gen(function* () {
      ...
      const uploadMedia = Effect.fn("ListingService.uploadMedia")(
        (params: {
          ...
          Effect.gen(function* () {
            const maybeListing = yield* repo.findById(params.listingId).pipe(Effect.orDie)
            const listing = yield* Option.match(maybeListing, {
              onNone: () =>
                Effect.fail(
                  new ListingNotFound({
            ...
            const url = yield* imageUpload.uploadFile(params.fileName, params.filePath)
      ...
      const deleteListing = Effect.fn("ListingService.delete")(
        (id: string, landlordId: string): Effect.Effect<void, ListingNotFound | ListingForbidden> =>
          Effect.gen(function* () {
            const maybeListing = yield* repo.findById(id).pipe(Effect.orDie)
            const listing = yield* Option.match(maybeListing, {
              onNone: () =>
