...
export class RateLimiter extends Context.Service<
  RateLimiter,
  {
    ...
    readonly checkRequest: (params: {
      ...
      limit: number
      windowSeconds: number
    }) => Effect.Effect<void, RateLimitExceeded, HttpServerRequest.HttpServerRequest>
  }
    ...
    Effect.gen(function* () {
      ...
      const check = (params: { key: string; limit: number; windowSeconds: number }) =>
        Effect.gen(function* () {
          ...
          if (ttl === -1) {
            yield* redis.expire(params.key, params.windowSeconds).pipe(Effect.catchTag("RedisError", () => Effect.void))
