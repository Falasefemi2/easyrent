    ...
    return handlers
      .handle("signUp", ({ payload }) =>
        Effect.gen(function* () {
          ...
          return yield* auth
            .signUp(payload)
            .pipe(Effect.catchTag("HashError", Effect.orDie), Effect.catchTag("EffectDrizzleQueryError", Effect.orDie))
        }),
      )
      ...
      .handle("signOut", ({ payload }) =>
        auth.signOut(payload.refreshToken).pipe(Effect.catchTag("EffectDrizzleQueryError", Effect.orDie)),
      )
  }),
).pipe(
