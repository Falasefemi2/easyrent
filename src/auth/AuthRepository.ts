...
export class AuthRepository extends Context.Service<
  AuthRepository,
  {
    ...
    readonly storeRefreshToken: (params: { userId: string; tokenHash: string; expiresAt: Date }) => DbEffect<void>
    readonly findRefreshToken: (tokenHash: string) => DbEffect<Option.Option<RefreshTokenRow>>
    ...
    readonly findByVerificationToken: (token: string) => Effect.Effect<Option.Option<UserRow>, EffectDrizzleQueryError>

    readonly markEmailVerified: (userId: string) => Effect.Effect<void, EffectDrizzleQueryError>
  }
