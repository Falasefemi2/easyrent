...
export class TokenService extends Context.Service<
  TokenService,
  {
    readonly signAccessToken: (payload: AccessTokenPayload) => Effect.Effect<string>
    readonly verifyAccessToken: (token: string) => Effect.Effect<AccessTokenPayload, InvalidToken | TokenExpired>
...
>()("easyrent/auth/TokenService") {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const generateRefreshToken = Effect.fn("TokenService.generateRefreshToken")(
        (): Effect.Effect<string> => Effect.sync(() => crypto.randomBytes(64).toString("hex")),
      )
