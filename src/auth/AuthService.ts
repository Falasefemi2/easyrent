...
type SignInError = InvalidCredentials | HashError | EffectDrizzleQueryError | EmailNotVerified
...
export class AuthService extends Context.Service<
  AuthService,
  {
    ...
    readonly signIn: (params: { email: string; password: string }) => Effect.Effect<AuthTokens, SignInError>

    readonly refresh: (rawRefreshToken: string) => Effect.Effect<AuthTokens, RefreshError>

    readonly signOut: (rawRefreshToken: string) => Effect.Effect<void, SignOutError>
    readonly verifyEmail: (token: string) => Effect.Effect<void, InvalidVerificationToken | TokenExpiredError>
  }
>()("easyrent/auth/AuthService") {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const issueTokens = Effect.fn("AuthService.issueTokens")((userId: string, email: string) =>
        Effect.gen(function* () {
        ...
        (params: {
          ...
          Effect.gen(function* () {
            ...
            yield* email
              ...
              .pipe(Effect.catch((e) => Effect.logWarning(`Failed to send verification email: ${JSON.stringify(e)}`)))
      ...
      const signIn = Effect.fn("AuthService.signIn")(
        (params: { email: string; password: string }): Effect.Effect<AuthTokens, SignInError> =>
          Effect.gen(function* () {
            ...
            const valid = yield* passwords.verify(user.passwordHash, params.password)
      ...
      const verifyEmail = Effect.fn("AuthService.verifyEmail")(
        (token: string): Effect.Effect<void, InvalidVerificationToken | TokenExpiredError> =>
          Effect.gen(function* () {
            const maybeUser = yield* repo.findByVerificationToken(token).pipe(Effect.orDie)
