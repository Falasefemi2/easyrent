  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return {
        ...
        accessTokenTtlSeconds: parseAccessTokenExpiry(config.ACCESS_TOKEN_EXPIRY),
        refreshTokenTtlDays: parseRefreshTokenExpiry(config.REFRESH_TOKEN_EXPIRY),
...
function parseAccessTokenExpiry(expiry: string): number {
  ...
  if (!match) throw new Error(`Invalid ACCESS_TOKEN_EXPIRY: "${expiry}". Expected format: 15m, 1h, 30s`)
...
function parseRefreshTokenExpiry(expiry: string): number {
  ...
  if (!match) throw new Error(`Invalid REFRESH_TOKEN_EXPIRY: "${expiry}". Expected format: 7d, 2w`)
  const value = parseInt(match[1]!, 10)
  switch (match[2]) {
    case "d":
      return value
