...
export class FavoritesRepository extends Context.Service<
  FavoritesRepository,
  {
    ...
    readonly findByUser: (userId: string, pagination: PaginationParams) => DbEffect<PaginatedResult<FavoriteListingRow>>
      ...
      const findByUser = Effect.fn("FavoritesRepository.findByUser")(
        (userId: string, pagination: PaginationParams): DbEffect<PaginatedResult<FavoriteListingRow>> =>
          Effect.gen(function* () {
            ...
            const listingRows = yield* withDbSpan(
            ...
            // Fetch media
            const mediaRows = yield* withDbSpan(
            ...
            // Fetch cover images
            const coverRows = yield* withDbSpan(
            ...
            const listingMap = Object.fromEntries(listingRows.map((r) => [r.id, r]))
