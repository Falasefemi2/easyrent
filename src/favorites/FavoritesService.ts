...
export class FavoritesService extends Context.Service<
  FavoritesService,
  {
    readonly add: (userId: string, listingId: string) => Effect.Effect<void, AlreadyFavorited>

    readonly remove: (userId: string, listingId: string) => Effect.Effect<void, FavoriteNotFound>
    ...
    readonly isFavorited: (userId: string, listingId: string) => Effect.Effect<boolean>
  }
    ...
    Effect.gen(function* () {
      ...
      const add = Effect.fn("FavoritesService.add")(
        (userId: string, listingId: string): Effect.Effect<void, AlreadyFavorited> =>
          Effect.gen(function* () {
            const already = yield* repo.exists(userId, listingId).pipe(Effect.orDie)
      ...
      const remove = Effect.fn("FavoritesService.remove")(
        (userId: string, listingId: string): Effect.Effect<void, FavoriteNotFound> =>
          Effect.gen(function* () {
            const exists = yield* repo.exists(userId, listingId).pipe(Effect.orDie)
      ...
      const getMyFavorites = Effect.fn("FavoritesService.getMyFavorites")(
        (userId: string, pagination: PaginationParams): Effect.Effect<PaginatedResult<FavoriteListingRow>> =>
