...
const ServicesLive = Layer.mergeAll(TokenService.layer, ImageUploadService.layer, EmailLive).pipe(
  Layer.provide(InfraLive),
)

// Repository layers that depend on DB
const RepositoriesLive = Layer.mergeAll(UsersRepository.layer).pipe(Layer.provide(InfraLive))
...
}).pipe(Layer.provide([AuthApiHandlers, UsersApiHandlers, ListingsApiHandlers, FavoritesApiHandlers]))
...
const HttpServerLayer = HttpRouter.serve(AllRoutes, {
  middleware: (app) =>
    app.pipe(
      ...
      HttpMiddleware.cors({
        allowedOrigins: ["http://localhost:3001", "https://easyrent-fe-eight.vercel.app"],
