...
const makeTestListingRepository = Layer.succeed(
  ...
  (() => {
    ...
    return {
      ...
      findById: (id) => Effect.succeed(Option.fromNullOr(listingsStore.get(id) ?? null)),
      ...
      findByLandlord: (landlordId, pagination) =>
        Effect.sync(() => {
          const all = Array.from(listingsStore.values()).filter((l) => l.landlordId === landlordId)
          const { page, limit } = pagination
          const offset = (page - 1) * limit
          const data = all.slice(offset, offset + limit)
          return {
...
const TestImageUploadService = Layer.succeed(ImageUploadService, {
  uploadFile: (_fileName, _filePath) => Effect.succeed("https://fake-cloudinary.com/test-image.jpg"),
})
  ...
  describe("getById", () => {
    ...
    it.effect("fails with ListingNotFound for unknown id", () =>
      Effect.gen(function* () {
        ...
        const result = yield* listingService.getById("non-existent-id").pipe(Effect.exit)
  ...
  describe("update", () => {
    it.effect("updates listing fields", () =>
      Effect.gen(function* () {
        ...
        const updated = yield* listingService.update(created.id, "landlord-123", {
          title: "New Title",
          price: "200000",
        })
    ...
    it.effect("fails with ListingForbidden for wrong landlord", () =>
      Effect.gen(function* () {
        ...
        const result = yield* listingService.update(created.id, "wrong-landlord", { title: "Hacked" }).pipe(Effect.exit)
  ...
  describe("delete", () => {
    it.effect("deletes listing successfully", () =>
      Effect.gen(function* () {
        ...
        const result = yield* listingService.getById(created.id).pipe(Effect.exit)
