...
export const makeTestFavoritesRepository = () =>
  Layer.succeed(
    FavoritesRepository,
    (() => {
      const favoritesStore = new Map<string, { userId: string; listingId: string; createdAt: string }>()
      ...
      return {
        ...
        exists: (userId, listingId) => Effect.succeed(favoritesStore.has(key(userId, listingId))),
        ...
        findByUser: (userId, pagination) =>
          Effect.sync(() => {
            ...
            const userFavs = Array.from(favoritesStore.values()).filter((f) => f.userId === userId)
        ...
        countForListing: (listingId) =>
          Effect.succeed(Array.from(favoritesStore.values()).filter((f) => f.listingId === listingId).length),
      }
    })(),
  )
...
describe("FavoritesService", () => {
  describe("add", () => {
    it.effect("adds a listing to favorites", () =>
      Effect.gen(function* () {
        ...
        const favorited = yield* favService.isFavorited("user-123", "listing-1")
    ...
    it.effect("fails with AlreadyFavorited on duplicate", () =>
      Effect.gen(function* () {
        const favService = yield* FavoritesService
        yield* favService.add("user-123", "listing-1")
        const result = yield* favService.add("user-123", "listing-1").pipe(Effect.exit)
  ...
  describe("remove", () => {
    ...
    it.effect("fails with FavoriteNotFound when not favorited", () =>
      Effect.gen(function* () {
        const favService = yield* FavoritesService
        const result = yield* favService.remove("user-123", "listing-2").pipe(Effect.exit)
