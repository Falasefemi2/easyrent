...
describe("RateLimiter", () => {
  it.effect("allows requests under the limit", () => {
    ...
    }).pipe(Effect.provide(RateLimiter.layer), Effect.provide(makeTestRedis(storage)), Effect.provide(mockLogger))
  })

  it.effect("fails when limit is exceeded and provides accurate retryAfter", () => {
    const storage = new Map<string, { value: number; ttl: number }>()
    return Effect.gen(function* () {
      const limiter = yield* RateLimiter
      ...
      const result = yield* limiter.check({ key: "test", limit: 2, windowSeconds: 60 }).pipe(Effect.flip)
