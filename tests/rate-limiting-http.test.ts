...
import { RateLimitExceeded, RateLimiter } from "../src/services/RateLimiter"
...
describe("HTTP SignUp Rate Limiting", () => {
  it.effect("returns 429 when rate limit is exceeded", () => {
    return Effect.gen(function* () {
      ...
      const client = yield* HttpApiClient.make(Api).pipe(Effect.provide(ApiRoutes))

      const result = yield* client.auth
        .signUp({
          payload: {
