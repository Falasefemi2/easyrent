...
import { AuthRepository, type RefreshTokenRow, type UserRow } from "../src/auth/AuthRepository"
...
const TestAuthConfig = Layer.succeed(AuthConfig, {
  accessTokenSecret: new TextEncoder().encode("test-access-secret-32-bytes-long!!"),
  refreshTokenSecret: new TextEncoder().encode("test-refresh-secret-32-bytes-long!"),
...
const makeTestAuthRepository = Layer.succeed(
  ...
  (() => {
    ...
    return {
      findByEmail: (email) => Effect.succeed(Option.fromNullOr(usersByEmail.get(email) ?? null)),

      findById: (id) => Effect.succeed(Option.fromNullOr(users.get(id) ?? null)),
      ...
      findRefreshToken: (tokenHash) => Effect.succeed(Option.fromNullOr(refreshTokens.get(tokenHash) ?? null)),
